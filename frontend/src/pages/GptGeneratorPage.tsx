...
const PRESET_SLOTS = [
  ...
  { code: 'ashutosh_homepage_728x90', width: 728, height: 90, label: '728x90 (Leaderboard)' },
  { code: 'ashutosh_homepage_970x250', width: 970, height: 250, label: '970x250 (Billboard)' },
  { code: 'ashutosh_mobile_320x50', width: 320, height: 50, label: '320x50 (Mobile Banner)' },
  { code: 'ashutosh_article_300x250', width: 300, height: 250, label: '300x250 (Article In-Content)' }
];

export const GptGeneratorPage: React.FC = () => {
  const [networkCode, setNetworkCode] = useState<string>(GAM_NETWORKS[0].code);
  const [adUnitCode, setAdUnitCode] = useState<string>(PRESET_SLOTS[0].code);
  const [width, setWidth] = useState<number>(300);
  const [height, setHeight] = useState<number>(250);
  const [divId, setDivId] = useState<string>(() => `div-gpt-ad-${PRESET_SLOTS[0].code.replace(/_/g, '-')}-${Date.now().toString().slice(-5)}`);
  
  // Tag Mode Selection: Standard Web GPT vs Google AMP vs PWA / Lazy-Load
  const [tagFormat, setTagFormat] = useState<'standard' | 'amp' | 'pwa' | 'hocalwire'>('standard');
